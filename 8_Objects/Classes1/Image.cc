...
Image::Image(const unsigned int width, const unsigned int hight)
    : m_width(width), m_height(hight),
      m_matrix(GrayscaleMatrix(m_width, std::vector<uchar>(m_height, 0)))
{
    std::cout << "Created image objact with shape = (" << m_width << "," << m_height << ")!"
              << std::endl;
}
...
unsigned int Image::get_heigt() const
{
    return m_height;
}
